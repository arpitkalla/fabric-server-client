...
void *cq_thread(void *arg)
{
    ...
    while (run)
    {
        ...
        {
            struct ctx *context = static_cast<ctx *>(comp.op_context);
...
int common_init(const char *addr, uint64_t flags)
{
    ...
    ret = fi_getinfo(FI_VERSION(1, 4), addr, PORT, flags, hints, &fi);
    if (ret)
    {
        perror("fi_getinfo");
        return ret;
...
{
    struct ctx *context = static_cast<ctx *>(arg);
    int i;
    ssize_t ret;
    for (i = 0; i < context->count; i++)
    {

        ret = fi_read(ep, buff, context->size, fi_mr_desc(mr),
                      0, keys.addr, keys.rkey, context);
        if (ret)
        {
            perror("fi_read");
            break;
...
int client(char *addr, int threads, int size, int count)
{
    ...
    for (i = 0; i < threads; i++)
    {
        ret = pthread_create(&context[i].thread, NULL,
                             client_thread, &context[i]);
        if (ret)
        {
            perror("pthread_create");
            return ret;
...
int main(int argc, char *argv[])
{
    ...
    context = static_cast<ctx *>(calloc(threads, sizeof(*context)));
    if (!context)
    {
        perror("calloc");
        return -1;
    ...
    for (i = 0; i < threads; i++)
    {
        pthread_mutex_init(&context[i].lock, NULL);
        pthread_cond_init(&context[i].cond, NULL);
        context[i].count = count;
        context[i].size = size;
    }
